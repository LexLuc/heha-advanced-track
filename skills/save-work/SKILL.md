...
7. **确认成功**：运行 `git log --oneline -1` 展示提交记录，告诉用户"工作已保存并备份到线上"，并展示远端仓库的PR链接

8. **等待用户合入**：提示用户去 GitHub 页面上合并 PR。等待用户确认已合入后，继续下一步

9. **切回主分支**：运行 `git checkout main && git pull origin main` 切回主分支并拉取最新代码
